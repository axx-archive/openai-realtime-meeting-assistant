...
## Founder decisions — RATIFIED 2026-07-11 (AJ, in-session)

All 13 open decisions were answered the same day. These are now plan constraints, not open items:

1. **Audio retention: YES — 72h rolling window** for committed-segment audio, with guest-visible disclosure. Unlocks: nightly gpt-4o-transcribe-diarize QA over low-confidence/failed segments; real meeting audio replaces the scripted corpus as STT ground truth (W2 corpus item upgraded accordingly).
2. **Spend alerts: SPEND_ALERT_DAILY_USD=75; rollup artifact visible to all roster users** (everyone-aware pillar).
3. **Private dashboard voice: browser usage beacon ships inside W0** — no unmetered lanes in the baseline.
4. **Live captions: none planned.** Single authoritative STT lane stands; no dual-lane caption work.
5. **Sonnet Sep-1 price step: eat it.** All Sonnet seats stay; no doctrine changes for cost. (W0 ledger still produces the real number for the record.)
6. **Opus 4.8 review/gate: RATIFIED as standing policy** while any non-Anthropic runner writes gated artifacts. Revisit only if the W2 shadow eval shows verdict parity with zero Opus-fail/Sonnet-pass defects. Refusal fallback stays Opus permanently.
7. **Voice-recall effort-floor carve-out: RATIFIED** — spoken-turn recall may run effort=low; seat-scoped exemption ships enabled in the W4 seat table (W5 item flips from "if ratified" to "build enabled").
8. **W4 doctrine parameters: all three as recommended** — strict model allowlist on frontier seats (orchestrator/deliverable); mixed-provenance runs allowed with provenance stamps; orchestrator effort-medium pilot covers orchestrator runs only (/goal engine single-shot calls stay at high).
9. **Channel kickoff: any signed-in member may confirm; legacy exact-keyword instant-launch is RETIRED** when propose-confirm parity ships (W5 gains an explicit retirement item).
10. **Suggestion-agent trust cap: standard lane only** — meeting-heard auto-proposals may never propose heavy-lane/external-write workflows.
11. **Standing approvals (069): proposed matrix RATIFIED** — morning brief + channel digests may auto_run (deterministic, no external writes); all model-written deliverables standard lane; external_write always heavy. The W4 standing-approvals surface builds against this matrix.
12. **Calendar: trigger meeting_prep off scheduled rooms + key dates now; Google OAuth deferred** to a post-W5 follow-on when it earns the build.
13. **Weekly memo: compile on schedule + permanently-parked heavy gate per send** — every investor send is one explicit founder tap with full funnel provenance.
