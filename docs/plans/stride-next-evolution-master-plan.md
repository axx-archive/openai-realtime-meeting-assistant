...
> being mislabeled as Video. The bounded Chat cold-load/crash successor now
> serves exact commit `d319c768...` at VPS ledger generation 71 from bundle
> `d8210f3f...`, app image `412b5ca8...` and renderer image `3cc8cebf...`;
> migration high-water remains 20, public health/readiness and
> `verified-local-unsigned` agree, and `7bf81f06...` is retained as rollback.
> iOS Build 60 (`b26bfeac...`, fingerprint `8d0a81d0...`) finished from that
> exact commit and submission `4b870fa5...` uploaded successfully to App Store
> Connect; Apple
> processing and intended TestFlight-group availability remain open. The
> author-only sent-message Project correction and linked-source invalidation
> boundary now passes its bounded local release-candidate gate. Resume with its
> exact reviewed release, then Project-linked attachment/reply source coverage
> before using permission-filtered
...
> for Builds 54-60 remain a release follow-up, not permission to reopen the accepted
...
- **Next:** ship and read back the accepted author-only sent-message Project
  correction candidate, then extend Project-linked source coverage through
  attachments and replies. Build the authorized recurring compaction and deterministic
