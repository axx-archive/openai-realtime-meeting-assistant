...
- The memory-bounded implementation boundary **A** is
  `b70b97990438e2b12cf11a26d9f2fa132d1bf4b4`. This plan is intentionally
...
- Two subsequent protected attempts passed progressively deeper gates and were
  ...
  Exact-A observation also now preserves runtime's established meaning for an
  optional lifecycle journal that has never been created: safe absence is an
  empty journal, normalization leaves it absent, and only the authorized repair
  append creates it. Symlink or other read failures still stop the ceremony.
