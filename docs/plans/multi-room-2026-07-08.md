...
- Expensive media (mixer, transcription lane, Realtime peer) becomes **lazy for every room including office**: created in `noteMeetingAdmission` on first seat, torn down after `endMeetingForIdle`'s 5-minute grace, fenced by a per-room `mediaGen` counter. Deliberate behavior change: ends today's always-on boot-started OpenAI Realtime spend. Scout voice starts concurrently with the join handshake so the first-join delay is invisible.
