...
- Amendment (2026-08-03, mobile-parity port of the ratified Table treatments): **the agent's authored voice also carries ember as identity** — the Scout bubble's hairline, its rich-text markers/quote rules, the workstream mark's ember-soft well (full ember only while queued/running), and the unread boundary (debt the reader owes the room). These are the ONLY ambient-ember exceptions; everything else stays earned.
