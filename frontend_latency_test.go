...
func TestIndexProvidesAuthenticatedWaveformHomeAndFloatingAssistant(t *testing.T) {
	...
	for _, want := range []string{
		...
		`id="homeRealtimeVoice" aria-label="Start a new private voice chat with Scout"`,
		`id="officeVoiceWaveform"`,
		...
		`data-research-terminal-actions`,
...
func TestIndexKeepsScreenShareTrackAndFullscreenPresentation(t *testing.T) {
	...
	for _, want := range []string{
		...
		"ScreenShareBlankFramesError",
		"ScreenShareOutboundStalledError",
