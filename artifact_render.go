...
func injectArtifactDeckNavigation(document string) string {
	const viewer = `<style id="bonfire-deck-viewer-style">html{background:#08080a}#stage{visibility:hidden}html[data-deck-ready="true"] #stage{visibility:visible}html:not([data-deck-ready="true"]) #deck-viewer-nav{visibility:hidden}#deck-viewer-nav{position:fixed;right:24px;bottom:20px;z-index:2147483647;display:flex;align-items:center;gap:10px;padding:8px 10px;border:1px solid rgba(255,255,255,.22);border-radius:999px;background:rgba(8,8,10,.78);color:#fff;font:600 14px/1 Arial,sans-serif;backdrop-filter:blur(12px)}#deck-viewer-nav button{width:36px;height:36px;border:0;border-radius:50%;background:rgba(255,255,255,.14);color:#fff;font:700 20px/1 Arial,sans-serif;cursor:pointer}#deck-viewer-nav button:focus-visible{outline:3px solid #fff;outline-offset:2px}@media print{#deck-viewer-nav{display:none!important}#stage{position:static!important;left:auto!important;top:auto!important;transform:none!important}#stage{visibility:visible!important}}</style><nav id="deck-viewer-nav" data-deck-navigation="trusted" aria-label="Slide navigation"><button type="button" data-deck-prev aria-label="Previous slide">&#8592;</button><span data-deck-position aria-live="polite"></span><button type="button" data-deck-next aria-label="Next slide">&#8594;</button></nav><script id="bonfire-deck-viewer-script">(()=>{'use strict';const pages=Array.from(document.querySelectorAll('#stage>.pg'));const stage=document.getElementById('stage');const status=document.querySelector('[data-deck-position]');let index=0;function show(next){if(!pages.length)return;index=Math.max(0,Math.min(pages.length-1,next));pages.forEach((page,i)=>{page.classList.toggle('on',i===index);page.setAttribute('aria-hidden',i===index?'false':'true')});status.textContent=(index+1)+' / '+pages.length}function fit(){if(!stage)return;const scale=Math.min(innerWidth/1920,innerHeight/1080);stage.style.position='absolute';stage.style.transform='scale('+scale+')';stage.style.left=((innerWidth-1920*scale)/2)+'px';stage.style.top=((innerHeight-1080*scale)/2)+'px';document.documentElement.dataset.deckReady='true'}document.querySelector('[data-deck-prev]').addEventListener('click',()=>show(index-1));document.querySelector('[data-deck-next]').addEventListener('click',()=>show(index+1));addEventListener('keydown',event=>{if(['ArrowRight','PageDown',' '].includes(event.key)){event.preventDefault();show(index+1)}else if(['ArrowLeft','PageUp'].includes(event.key)){event.preventDefault();show(index-1)}else if(event.key==='Home'){show(0)}else if(event.key==='End'){show(pages.length-1)}});addEventListener('resize',fit);show(0);fit()})();</script>`
