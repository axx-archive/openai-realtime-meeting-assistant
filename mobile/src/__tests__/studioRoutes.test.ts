...
test('documents expose Edit and full-screen actions while unsupported artifacts stay read-only', async () => {
  registerTestStubModules('studio-preview-stub:', {
    ...
    'studio-preview-stub:../config': `export const API_BASE_URL='https://example.test'; export const NATIVE_CLIENT_HEADER='expo';`,
    'studio-preview-stub:../api/requestHelpers': `export const buildApiUrl=(_base,path)=>path; export const buildAuthHeaders=()=>({});`,
    'studio-preview-stub:../artifacts/nativeDeckViewer': `export const nativeDeckPreviewPath=()=>null; export const nativeTextArtifactIsRenderable=value=>{const text=String(value??'').trim();if(!text||text.startsWith('<'))return false;try{JSON.parse(text);return false}catch{return true}};`,
