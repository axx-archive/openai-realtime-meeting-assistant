...
test('generic deliverables use only the closed save-only Drive capability', () => {
  assert.match(client, /artifactDriveSaveCapability\(sessionToken: string\)/u);
  ...
  assert.match(screen, /api\.artifactDriveSaveCapability\(sessionToken\)/u);
  assert.match(screen, /capability\.action === 'save' && capability\.receiptBacked === true/u);
