...
test('Home is voice-first and has no competing composer or duplicate navigation cluster', () => {
  const canvas = source('src', 'screens', 'CanvasScreen.tsx');
  assert.match(canvas, /<StrideCradle/);
  assert.match(canvas, /usePersonalRealtime/);
  assert.doesNotMatch(canvas, /Message Scout|composerDock|useComposerDictation|<ChatCircle|<NavCluster/);
});
