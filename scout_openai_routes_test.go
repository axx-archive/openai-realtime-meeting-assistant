...
func TestScoutCoreRouteModelsAreExplicitOpenAISeats(t *testing.T) {
	...
	if got := scoutRouterModel(); got != "gpt-5.6-terra" {
		t.Fatalf("router model=%q, want Terra", got)
	}
...
func TestScoutCoreIgnoresInstalledAnthropicKey(t *testing.T) {
	...
	swapOpenAITextResponder(t, func(_ context.Context, apiKey string, request openAITextRequest) (string, error) {
		...
		case "scout_route":
			if request.Model != defaultScoutRouterModel || request.JSONSchema == nil || request.Seat != seatRouter {
				t.Fatalf("router request=%+v, want Terra strict route seat", request)
			}
			return openAIScoutRouteJSON(t, openAIScoutRouterOutput{Route: "inline"}), nil
		case "scout_chat":
			if request.Model != defaultScoutChatModel || request.Seat != seatChat {
				t.Fatalf("chat request=%+v, want Terra chat seat", request)
			}
