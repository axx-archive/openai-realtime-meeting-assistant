...
(async()=>{
 ...
 await page.waitForFunction(()=>document.querySelector('#channel-checkpoint-fixture .scout-chat-work-card__checkpoint')?.dataset.state==='selected');
	...
	assert.deepEqual(studioLabels,['Approve external sharing','Request rebuild','Keep internal','Franchise playbook']);
	await page.evaluate(()=>{
	...
	assert.deepEqual(await legacyStudio.locator('.scout-chat-work-card__checkpoint-choice').allTextContents(),['Approve external sharing','Keep internal']);
	await page.evaluate(()=>{
