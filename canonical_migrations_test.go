...
func TestLoadCanonicalMigrations(t *testing.T) {
	...
	if migrations[18].Version != 19 || migrations[18].Name != "0019_stride_project_chat_send.sql" ||
		migrations[18].SHA256 != sha256.Sum256([]byte(migrations[18].SQL)) ||
		!strings.Contains(migrations[18].SQL, "CREATE TABLE stride_project_chat_send_receipts") ||
		!strings.Contains(migrations[18].SQL, "Project chat Send requires current organization session authority") ||
	...
	if migrations[24].Version != 25 || migrations[24].Name != "0025_source_episode_ledger.sql" ||
		migrations[24].SHA256 != sha256.Sum256([]byte(migrations[24].SQL)) ||
		!strings.Contains(migrations[24].SQL, "CREATE TABLE stride_source_episode_revisions") ||
		!strings.Contains(migrations[24].SQL, "stride_source_episode_purge_trigger") {
		t.Fatalf("unexpected SourceEpisode ledger migration: %+v", migrations[24])
	}
