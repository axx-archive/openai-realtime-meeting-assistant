...
func assistantRealtimeMilestoneHandler(w http.ResponseWriter, r *http.Request) {
	...
	hasBinding := strings.TrimSpace(payload.VoiceSessionID) != "" || strings.TrimSpace(payload.ThreadID) != "" || payload.TransportRevision != 0 || strings.TrimSpace(payload.OperationID) != "" || strings.TrimSpace(payload.LeaseToken) != "" || payload.LeaseGeneration != 0
	if hasBinding {
		if kanbanApp == nil || strings.TrimSpace(payload.VoiceSessionID) == "" || strings.TrimSpace(payload.ThreadID) == "" || payload.TransportRevision <= 0 || strings.TrimSpace(payload.OperationID) == "" || strings.TrimSpace(payload.LeaseToken) == "" || payload.LeaseGeneration <= 0 {
			writeAuthError(w, http.StatusBadRequest, "realtime milestone binding is incomplete")
			return
		}
		latency, replayed, err := kanbanApp.appendPrivateRealtimeVoiceLeaseTransportMilestone(user.Email, strideE10SessionHashFromRequest(r), payload.VoiceSessionID, payload.ThreadID, payload.LeaseToken, payload.LeaseGeneration, payload.TransportRevision, payload.OperationID, milestone, time.Now().UTC())
		if err != nil {
