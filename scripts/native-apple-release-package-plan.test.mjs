...
assert.match(plan.archives.macos, /STRIDE-macOS\.xcarchive$/);
assert.match(plan.exports.macosApp, /STRIDE\.app$/);
assert.match(plan.exports.macosDmg, /STRIDE-1\.0\.dmg$/);
assert.match(plan.commands.macCreateDmg.shell, /scripts\/build-macos-dmg\.sh/);
assert.match(plan.commands.macCreateDmg.shell, /STRIDE_APP_PATH=.*STRIDE\.app/);
assert.match(plan.commands.macCreateDmg.shell, /STRIDE_DMG_SIGN_IDENTITY="\$STRIDE_DMG_SIGN_IDENTITY"/);
assert.match(plan.commands.macSubmitNotary.shell, /"\$NOTARYTOOL_KEYCHAIN_PROFILE"/);
assert.match(plan.commands.macSubmitNotary.shell, /STRIDE-1\.0\.dmg/);
assert.match(plan.commands.macStapleDmg.shell, /stapler staple .*STRIDE-1\.0\.dmg/);
assert.match(plan.commands.macValidateStapledDmg.shell, /stapler validate .*STRIDE-1\.0\.dmg/);
assert.match(plan.commands.macVerifySignedDmg.shell, /codesign --verify --strict/);
assert.match(plan.commands.macGatekeeperDmg.shell, /--type open --context context:primary-signature/);
assert.match(plan.commands.macMountDmgForVerification.shell, /hdiutil attach -nobrowse -readonly/);
assert.match(plan.commands.macGatekeeperMountedApp.shell, /--type execute .*STRIDE\.app/);
assert.match(plan.commands.macDetachDmgVerificationMount.shell, /hdiutil detach/);
assert.doesNotMatch(JSON.stringify(plan.commands), /MeetingAssistMacApp\.zip|MeetingAssist\.app/);
...
assert.equal(plan.commands.promoteMacMediaEvidence, undefined);
