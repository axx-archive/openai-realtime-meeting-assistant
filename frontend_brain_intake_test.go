...
func TestIndexBrainIntakePlaceholderIsGuided(t *testing.T) {
	...
	if !strings.Contains(body, "message the channel") || strings.Contains(body, "message the channel — @scout to ask") || !strings.Contains(body, "`ask ${directAgent || 'Scout'}, or tap + to run a task`") {
