...
func TestIndexBrainIntakePlaceholderIsGuided(t *testing.T) {
	...
	// The other two placeholders survive, with the private copy inviting one
	// natural-language surface rather than a tool launcher.
	if !strings.Contains(body, "message the channel") || strings.Contains(body, "message the channel — @scout to ask") || !strings.Contains(body, "`ask ${directAgent || 'Scout'} anything`") {
